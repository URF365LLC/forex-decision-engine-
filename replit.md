...
1. **Indicator Service** (`indicatorService.ts`) - Fetches OHLCV and technical indicators from Alpha Vantage
2. **Trend Filter** (`trendFilter.ts`) - Higher timeframe trend direction using EMA 200 + ADX
3. **Entry Trigger** (`entryTrigger.ts`) - Pullback detection to EMA 20/50 zone with RSI confirmation
4. **Position Sizer** (`positionSizer.ts`) - Risk-based lot sizing with prop firm constraints
5. **Grader** (`grader.ts`) - Confluence scoring producing A+/B/no-trade grades
6. **Decision Engine** (`decisionEngine.ts`) - Orchestrates all components into final trade signals
...
- **Design**: Mobile-first, dark theme CSS
