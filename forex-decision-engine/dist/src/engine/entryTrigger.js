...
import { getLatestValue, getPreviousValue } from './indicatorService.js';
import { createLogger } from '../services/logger.js';
const logger = createLogger('EntryTrigger');
// ═══════════════════════════════════════════════════════════════
// ENTRY ANALYSIS
// ═══════════════════════════════════════════════════════════════
export function analyzeEntry(data, trendDirection) {
    ...
    if (trendDirection === 'bullish') {
        ...
        const rsiHistory = [rsiPrevious, rsi2Back, rsi3Back, rsi4Back].filter((v) => v !== null);
    ...
    // BUILDING: In tolerance zone OR in strict zone but RSI not ready
        ...
        if (!rsiWasReset)
            missing.push('RSI not reset');
        if (!rsiTurning)
            missing.push('RSI not turning');
...
// HELPERS
// ═══════════════════════════════════════════════════════════════
function createInvalidResult(price, ema20, ema50, rsi, reason) {
    return {
        status: 'invalid',
        price,
        ema20: ema20 ?? 0,
...
//# sourceMappingURL=entryTrigger.js.map