 ...
 */
import { alphaVantage } from '../services/alphaVantageClient.js';
import { STRATEGY, getStyleConfig } from '../config/strategy.js';
 ...
 */
export async function fetchIndicators(symbol, style) {
 ...
 */
export function getLatestValue(indicators) {
 ...
 */
export function getPreviousValue(indicators, offset = 1) {
    if (indicators.length <= offset)
        return null;
 ...
 */
export function calculateSlope(indicators, periods) {
    if (indicators.length < periods + 1)
        return 0;
 ...
 */
export function findSwingHigh(bars, lookback) {
 ...
 */
export function findSwingLow(bars, lookback) {
    if (bars.length < lookback)
        return null;
    const recentBars = bars.slice(-lookback);
