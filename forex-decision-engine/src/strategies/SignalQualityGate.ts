...
export function isTrendAligned(trend: H4TrendResult, direction: 'long' | 'short'): boolean {
  if (trend.direction === 'neutral') return true;
...
function checkSession(symbol: string): SessionResult {
  ...
  switch (instrumentClass) {
    case 'fx':
      ...
      if (utcHour >= 13 && utcHour < 17) return { allowed: true, adjustment: 10 };
      if (utcHour >= 8 && utcHour < 16) return { allowed: true, adjustment: 5 };
    ...
    case 'stocks':
      ...
      if (utcHour === 13) return { allowed: true, adjustment: -5 };
      if (utcHour >= 14 && utcHour < 16) return { allowed: true, adjustment: 5 };
      return { allowed: true, adjustment: 0 };
