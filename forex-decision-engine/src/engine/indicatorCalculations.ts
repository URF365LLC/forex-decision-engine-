 ...
 * Used by cryptoIndicatorService and twelveDataIndicatorService
 * Implements Wilder's smoothing for RSI/ATR/ADX
 */

import { OHLCVBar, IndicatorValue } from '../services/alphaVantageClient.js';

export function calculateEMA(bars: OHLCVBar[], period: number): IndicatorValue[] {
...
export function calculateRSI(bars: OHLCVBar[], period: number = 14): IndicatorValue[] {
...
export function calculateATR(bars: OHLCVBar[], period: number = 14): IndicatorValue[] {
  if (bars.length < period + 1) return [];
...
export function calculateADX(bars: OHLCVBar[], period: number = 14): IndicatorValue[] {
